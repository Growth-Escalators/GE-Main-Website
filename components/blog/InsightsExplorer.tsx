...
export default function InsightsExplorer({ posts }: InsightsExplorerProps) {
  ...
  const filtered = useMemo(() => {
    ...
    return posts.filter((post) => {
      ...
      const queryMatch = !needle || [post.title, post.description, post.categoryLabel, post.contentTypeLabel, post.author, ...post.tags].join(' ').toLowerCase().includes(needle)
      ...
      <div className={styles.explorer}>
        <aside className={styles.filters} aria-label="Filter insights">
          <div className={styles.searchBlock}>
            ...
            <input id="insights-search" type="search" placeholder="Search topics, channels, problems…" value={query} onChange={(event) => setQuery(event.target.value)} />
          </div>

          <div className={styles.filterGroup}>
            <p>Topics</p>
            <button type="button" className={category === ALL ? styles.active : undefined} onClick={() => setCategory(ALL)}>
              <span>All topics</span><small>{posts.length}</small>
            </button>
            {categories.map(([name, count]) => (
              <button type="button" key={name} className={category === name ? styles.active : undefined} onClick={() => setCategory(name)}>
          ...
          <div className={styles.filterGroup}>
            <p>Resources</p>
            <button type="button" className={format === ALL ? styles.active : undefined} onClick={() => setFormat(ALL)}>
              <span>All formats</span><small>{posts.length}</small>
            </button>
            {formats.map(([name, count]) => (
              <button type="button" key={name} className={format === name ? styles.active : undefined} onClick={() => setFormat(name)}>
                <span>{name}</span><small>{count}</small>
              </button>
            ))}
          </div>

          {activeFilters && <button type="button" className={styles.clear} onClick={clearFilters}>Clear filters ↺</button>}
        </aside>
            ...
            {filtered.map((post, index) => (
              <article className={styles.card} key={post.slug}>
                <Link href={`/blog/${post.slug}`} className={styles.visualLink} aria-label={`Read ${post.title}`}>
                  <EditorialVisual title={post.title} category={post.categoryLabel} typeLabel={post.contentTypeLabel} tone={post.gradient} compact />
                </Link>
                <div className={styles.cardBody}>
                  <div className={styles.meta}><span>{post.categoryLabel}</span><span>{post.contentTypeLabel}</span></div>
                  <Link href={`/blog/${post.slug}`} className={styles.titleLink}><h3>{post.title}</h3></Link>
