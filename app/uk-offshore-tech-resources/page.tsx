...
import {
  ...
  EngagementModels, ComparisonTable, FaqAccordion, FaqPageJsonLd, LeadForm, DisclosureStrip,
  CtaButton,
} from '@/components/landing/international'
import { trackLandingEvent } from '@/lib/analytics'
import { UK_CONTENT } from '@/lib/content/international-landing/uk'
...
/* ═════════════════════════════════════════════════════════════════════════
   ...
   component family (see the Stage A report for why): "sample profiles" lead
   magnet band, "what you receive" proof section, and the "clear terms" risk
   -reduction strip. All three are simple, single-use content blocks that
   don't repeat their exact structure anywhere else in the shared list this
   Stage was briefed to build, so they're kept here, driven by `UK_CONTENT`
   the same way the shared sections are — a future market page can copy this
   file's pattern directly, or these can be promoted to shared components
   later if UAE/US/Australia turn out to want the identical structure.
   ═════════════════════════════════════════════════════════════════════════ */
...
/* ═════════════════════════════════════════════════════════════════════════
   ...
   UK_CONTENT. Every other market's page.tsx should follow this exact shape:
   import its own content object + the same shared components, swap
   UK_CONTENT for e.g. UAE_CONTENT, and add/remove page-local sections above
   as needed.
   ═════════════════════════════════════════════════════════════════════════ */
