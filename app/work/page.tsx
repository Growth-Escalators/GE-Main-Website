      ...
      <main>
        ...
        <section ref={heroRef} className="pt-36 pb-20 px-6 md:px-12 lg:px-24" style={{ background: 'var(--bg-primary)' }}>
          <div className="max-w-7xl mx-auto work-hero-content">
            ...
            <h1 className="font-extrabold leading-none mb-5" style={{ fontSize: 'clamp(44px, 8vw, 100px)', color: 'var(--text-primary)' }}>
              Work That Speaks<br />For Itself
            </h1>
            <p className="font-light text-xl max-w-xl" style={{ color: 'var(--text-muted)', lineHeight: 1.75 }}>
            ...
            <div className="flex flex-wrap gap-2">
              {FILTERS.map((f) => (
                <button
                  ...
                  className="relative text-sm px-5 py-2 transition-all duration-300"
                  style={{
              ...
              {filtered.map((cs) => (
                <div key={cs.id} className="case-card ge-card p-8 group hover:-translate-y-1 transition-transform duration-300">
                  ...
                  <div className="flex items-start justify-between mb-6">
                    <div>
                      <h3 className="font-extrabold font-bold text-xl mb-1" style={{ color: 'var(--text-primary)' }}>
                        {cs.name}
                      </h3>
                      <span
                        className="text-[10px] uppercase tracking-widest px-2 py-1"
                        style={{ color: 'var(--orange)', background: 'rgba(255, 107, 53,0.08)' }}
                      >
                        {cs.industry}
                      </span>
                    </div>
                    <div className="w-12 h-12 rounded-full flex items-center justify-center font-extrabold font-bold shrink-0" style={{ background: cs.gradient, fontSize: 14, color: '#F0EDE8' }}>
                      {cs.name[0]}
                    </div>
                  </div>
                  ...
                  <div className="space-y-3 mb-6">
                    <div>
                      <span className="text-[10px] uppercase tracking-widest block mb-1" style={{ color: 'var(--text-muted)', opacity: 0.6 }}>Challenge</span>
                      <p className="text-sm" style={{ color: 'var(--text-muted)' }}>{cs.challenge}</p>
                    </div>
                    <div>
                      <span className="text-[10px] uppercase tracking-widest block mb-1" style={{ color: 'var(--text-muted)', opacity: 0.6 }}>Solution</span>
                      <p className="text-sm" style={{ color: 'var(--text-muted)' }}>{cs.solution}</p>
                    </div>
                  </div>
                    ...
                    {cs.results.map((r, i) => {
                      ...
                      return (
                        <div key={i} className="p-3 text-center" style={{ background: 'var(--bg-secondary)', border: '1px solid var(--border-subtle)' }}>
                          <div className="font-bold tracking-[0.02em] text-xl leading-none mb-1" style={{ color: 'var(--orange)' }}>
                            {displayVal}
                          </div>
                          <div className="text-[10px] uppercase tracking-wider leading-tight" style={{ color: 'var(--text-muted)' }}>
                            {r.metric}
                          </div>
                        </div>
                      )
                  ...
                  <div className="flex flex-wrap gap-2 mb-6">
                    {cs.tags.map((tag) => (
                      <span key={tag} className="text-[10px] uppercase tracking-wider px-2 py-1" style={{ color: 'var(--orange)', border: '1px solid rgba(255, 107, 53,0.2)' }}>
                        {tag}
                      </span>
                    ))}
                  </div>

                  <div className="text-sm transition-colors duration-300" style={{ color: 'var(--orange)' }}>
