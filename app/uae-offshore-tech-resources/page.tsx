...
import { UAE_CONTENT } from '@/lib/content/international-landing/uae'
...
/* ═════════════════════════════════════════════════════════════════════════
   ...
   component family, following the exact pattern established by
   ...
   -reduction strip, all driven by UAE_CONTENT.

   WhatsAppBand is UAE-only (not part of the UK pattern) — see the build
   report for why: a verified GE business WhatsApp number already exists
   and is used site-wide (wa.me/917733888883, e.g. app/contact/page.tsx,
   components/ui/GrowthBot.tsx, components/sections/LeadFormBand.tsx), so
   the optional "WhatsApp Our Team" CTA named in the UAE brief is wired up
   for real rather than omitted. It fires its own `uae_lp_whatsapp_click`
   event locally, since no shared component owns this action.
   ═════════════════════════════════════════════════════════════════════════ */
...
/**
 * UAE-only. A verified GE business WhatsApp number (+91 77338 88883,
 * wa.me/917733888883) is already used site-wide as a real lead channel —
 * see app/contact/page.tsx, components/ui/GrowthBot.tsx and
 * components/sections/LeadFormBand.tsx. No shared international-landing
 * component owns a WhatsApp action, so this fires `uae_lp_whatsapp_click`
 * itself via trackLandingEvent, per the brief's instruction for this exact
 * situation. Deliberately a static inline band (not a floating bubble or
 * chat-widget simulation) to stay inside the visual-system constraints.
 */
function WhatsAppBand() {
  const WA_NUMBER = '917733888883'
  const waHref = `https://wa.me/${WA_NUMBER}?text=${encodeURIComponent(
    "Hi Growth Escalators, I'd like to discuss an offshore technology requirement for our UAE team.",
  )}`
  return (
    <section aria-label="WhatsApp contact" style={{ background: 'var(--bg-secondary)', borderTop: '1px solid var(--border-hair)' }}>
      <div
        className="container-x flex flex-col sm:flex-row items-center justify-between text-center sm:text-left"
        style={{ padding: 'clamp(28px, 5vw, 44px) clamp(20px, 4vw, 40px)', gap: 20 }}
      >
        <div className="flex flex-col sm:flex-row items-center" style={{ gap: 12 }}>
          <span
            className="inline-flex items-center justify-center flex-shrink-0"
            style={{ width: 40, height: 40, borderRadius: '50%', background: 'rgba(0,212,170,0.14)', color: 'var(--teal-dark)' }}
          >
            <MessageCircle size={19} />
          </span>
          <div>
            <div style={{ fontSize: 15.5, fontWeight: 800, color: 'var(--text-primary)' }}>Prefer WhatsApp?</div>
            <div style={{ fontSize: 13.5, color: 'var(--text-secondary)' }}>Message our team directly for a quick response.</div>
          </div>
        </div>
        <CtaButton
          href={waHref}
          ...
          onClick={() => trackLandingEvent(c.eventPrefix, 'whatsapp_click', { location: 'whatsapp_band' })}
        />
      </div>
    </section>
  )
...
/* ═════════════════════════════════════════════════════════════════════════
   ...
   UAE_CONTENT, following the exact shape of
   app/uk-offshore-tech-resources/page.tsx. Section order matches the
   universal brief (Hero, Trust, Pain, Skills, Time-zone, Screening, How it
   works, Engagement models, Sample profiles, Proof, Risk, Comparison,
   FAQs, Form, Footer), with the UAE-only WhatsApp band inserted just above
   the requirement form as a lighter-weight alternative contact path.
   ═════════════════════════════════════════════════════════════════════════ */

export default function UAEOffshoreTechResourcesPage() {
  return (
    <>
